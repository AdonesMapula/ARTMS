...
export default function Jobs() {
  ...
  // ── helpers: API returns department_name not name ─────────────────────────
  const deptName = (j) =>
    j.department?.department_name ?? j.department?.name ?? "";

  const departments = useMemo(() => {
    const set = new Set(jobs.map(deptName).filter(Boolean));
  ...
  const filtered = useMemo(() => {
    ...
    return jobs.filter((j) => {
      const dn = deptName(j).toLowerCase();
      const matchesQuery =
        !q ||
        j.job_library?.job_title?.toLowerCase().includes(q) ||
        dn.includes(q) ||
        j.location?.toLowerCase().includes(q);
      const matchesDept = dept === "all" || deptName(j) === dept;
        ...
        <div className="relative mx-auto max-w-7xl">
          <div className="flex flex-col items-center text-center">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 backdrop-blur-sm">
              ...
              <p className="text-xs font-bold uppercase tracking-wider text-white">Career Opportunities</p>
            </div>
            ...
            <div className="mt-10 grid w-full max-w-2xl grid-cols-2 gap-4 sm:grid-cols-3">
              ...
              <div className="col-span-2 rounded-2xl bg-white/10 px-6 py-4 backdrop-blur-sm sm:col-span-1">
                ...
                <p className="mt-1 text-sm text-slate-300">Online Apply</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-12 lg:px-10">
        ...
        <div className="relative -mt-16 rounded-2xl border border-slate-200/50 bg-white p-6 shadow-xl shadow-slate-900/5 backdrop-blur-sm">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold uppercase tracking-wider text-[#111A62]">Filter &amp; Search</h2>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-[1fr,auto,auto]">
            ...
            <div className="flex items-end">
              <Button
                ...
                onClick={() => { setQuery(""); setDept("all"); setPage(1); }}
              >
                <FiFilter aria-hidden="true" /> Reset
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 flex items-center justify-between">
          <p className="text-sm text-slate-600">
            Showing <span className="font-bold text-[#111A62]">{total}</span> opening{total === 1 ? "" : "s"}
          </p>
            ...
            {pageItems.map((job) => (
              ...
              >
                <CardHeader className="relative">
                  <div className="absolute right-0 top-0 h-20 w-20 translate-x-10 -translate-y-10 rounded-full bg-[#F97316]/5 transition-transform duration-300 group-hover:translate-x-6 group-hover:-translate-y-6" />
                  <div className="relative flex flex-wrap items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate-600">
                        <span className="inline-flex items-center gap-1.5">
                          ...
                          <span className="font-medium">{deptName(job) || "N/A"}</span>
                        </span>
                        {job.location && (
                          <span className="inline-flex items-center gap-1.5">
                    ...
                    <Badge tone="accent" className="bg-[#F97316]/10 text-[#F97316] ring-1 ring-[#F97316]/20">
                ...
                <CardContent>
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    <Button as={Link} to={`/jobs/${job.id}`} variant="primary" className="bg-[#111A62] hover:bg-[#0d1550]">
                      View Details
                    </Button>
                    <Button as={Link} to={`/apply/${job.id}`} variant="outline" className="border-[#F97316] text-[#F97316] hover:bg-[#F97316] hover:text-white">
                      Apply Now
                    </Button>
                  </div>
                </CardContent>
