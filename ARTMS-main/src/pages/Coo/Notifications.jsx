...
export default function CooNotifications() {
  ...
  const stats = useMemo(() => {
    ...
    notifications.forEach((n) => {
      ...
      if (title.includes("approve")) approved++;
    ...
    <div className="space-y-6 font-sans">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          A complete timeline of your executive approvals, revisions, and system notifications.
        </p>
      </div>
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
        ...
        <Card className="bg-emerald-50/50 border-emerald-100">
          <CardContent className="p-4 flex flex-col items-center justify-center text-center">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-500 mt-1">Approved</span>
          </CardContent>
        </Card>
      ...
      <Card className="shadow-lg border-slate-200 overflow-hidden rounded-2xl">
        <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <FiBookOpen className="text-slate-500" size={18} />
            <h3 className="text-sm font-extrabold text-slate-800">Timeline</h3>
          </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-slate-400 gap-3">
              ...
              <p className="text-xs text-slate-400">Your recent approvals and notifications will appear here.</p>
            </div>
          ) : (
            <div className="relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent">
              <div className="space-y-8">
                {notifications.map((n) => (
                  <div key={n.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    ...
                    {/* Card */}
                    <div className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] rounded-2xl border p-4 shadow-sm transition-all duration-300 hover:shadow-md ${n.read ? 'bg-white border-slate-200' : 'bg-blue-50/30 border-blue-200 ring-1 ring-blue-500/10'}`}>
