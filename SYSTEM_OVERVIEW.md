...
| # | Template File | Trigger Scenario | Target Recipient | Content & Key Functionality |
| :--- | :--- | :--- | :--- | :--- |
| 1 | `user-created.blade.php` | Admin creates user account | New User / Staff | Temporary password, secure account setup button, initial login guidance. |
| 2 | `otp.blade.php` | Forgot password initiated | Registered User | 6-digit bold OTP security code, 10-minute expiry warning, security advice. |
| 3 | `interview_invitation.blade.php` | Interview scheduled by HR | Job Applicant | Dynamic stage badge (Initial, Second, Final), scheduled date/time, LiveKit video room link, HR contact email. |
| 4 | `interview_reminder.blade.php` | Automated/manual interview reminder | Job Applicant | Schedule confirmation, video room link, camera/microphone readiness tips. |
| 5 | `screening_rejection.blade.php` | Candidate fails screening or rejected | Job Applicant | Professional rejection message, constructive feedback, talent pool archiving notice. |
| 6 | `alternative_role_recommendation.blade.php` | AI detects candidate fits other open roles | Job Applicant | Screening feedback accompanied by AI-matched vacancies and one-click application links. |
| 7 | `system-notification.blade.php` | High-priority system event or approval alert | Internal Staff | Event title, summary description, category badge (alert, approval, info), CTA button to app URL. |
| 8 | `ready_for_interview.blade.php` | Applicant shortlisted for interview stage | Job Applicant | Status notification confirming successful screening and upcoming interview scheduling. |
| 9 | `hired.blade.php` | Candidate status updated to "Hired" | Candidate & HR | Formal hiring congratulatory notice, onboarding schedule, start date, HR contact details. |
| 10 | `new_application.blade.php` | Candidate submits job application | Candidate & HR | Confirmation of receipt, unique Application Tracking Code, next steps roadmap. |
