...
class GeminiService
{
        ...
        foreach ($keys as $keyIndex => $apiKey) {
            ...
            foreach (self::$models as $model) {
                try {
                    ...
                    $effectiveMaxTokens = max($maxTokens, 8192);
    ...
    protected static function extractTextFromResponse(?array $responseData): ?string
    {
        ...
        // Find the part that has a 'text' key but NOT a 'thoughtSignature' key
        // (thinking models return: [{text: "...", thoughtSignature: "..."}] or [{thought: true, text: "..."}, {text: "..."}])
        $textParts = [];
