    ...
    public function run(): void
    {
        ...
        if ($applicants->count() >= 1) {
            ...
            $records[] = Interview::create([
                ...
                // New schema-aligned stage values (interview_stage is now VARCHAR)
                'interview_stage'    => 'initial_screening',
                // New schema enum: initial, technical, hr, final, ai_screening
                ...
                'reminder_sent'      => false,
                'hr_decision'        => 'pending',
                'applicant_name'     => trim("{$a->first_name} {$a->last_name}"),
                'applicant_email'    => $a->email,
            ]);
            $a->update(['status' => 'interview_scheduled', 'current_stage' => 'Initial Screening']);
        }
        ...
        if ($applicants->count() >= 3) {
            ...
            $records[] = Interview::create([
                ...
                'hr_decision'       => 'pass',
                'ai_summary'        => 'Candidate demonstrated excellent communication and a strong grasp of technical requirements. Cultural alignment is evident. Recommended to proceed to the next round.',
                'ai_recommendation' => 'Proceed to HR Interview.',
                'interviewer_notes' => 'Impressed with system design knowledge. Minor gaps in cloud architecture.',
                'feedback'          => 'Strong candidate — proceed to HR interview.',
                'rating'            => 4,
                'applicant_name'    => trim("{$a->first_name} {$a->last_name}"),
                'applicant_email'   => $a->email,
            ]);
            $a->update(['status' => 'interview_completed', 'current_stage' => 'Technical Interview']);
        }
        ...
        if ($applicants->count() >= 4) {
            ...
            $a->update(['status' => 'interview_scheduled', 'current_stage' => 'HR Interview']);
        }
        ...
        if ($applicants->count() >= 5) {
            ...
            $a->update(['status' => 'interview_scheduled', 'current_stage' => 'Final Interview']);
        }
        ...
        if ($applicants->count() >= 6) {
            ...
            $records[] = Interview::create([
                ...
                'hr_decision'       => 'pass',
                'ai_summary'        => 'Candidate demonstrated adequate communication. Technical knowledge is satisfactory. Recommend for human interview.',
                'ai_recommendation' => 'Proceed to Initial Screening with HR.',
                'duration_minutes'  => 20,
                'applicant_name'    => trim("{$a->first_name} {$a->last_name}"),
                'applicant_email'   => $a->email,
            ]);
            $a->update(['status' => 'screening', 'current_stage' => 'AI Screening']);
        }
        ...
        $this->command->table(
            ...
            collect($records)->map(fn($i) => [
                ...
                $i->interview_type,
                $i->status,
                $i->applicant_name ?? optional($i->applicant)->first_name,
